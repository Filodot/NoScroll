...
object StaticLearningCatalog {
    ...
    val firstLesson = LessonPackage(
        ...
        introduction = "Переменная в Python связывает понятное имя со значением. " +
            "Например, запись age = 18 создаёт имя age и связывает его с числом 18. " +
            "Знак = выполняет присваивание, тогда как == используется для сравнения двух значений.\n\n" +
            "Выражение может читать уже созданную переменную и вычислять новое значение. " +
            "После x = 4 выражение x * 2 даст 8, потому что программа подставит текущее значение x. " +
            "Результат можно передать в print, чтобы увидеть его на экране.\n\n" +
            "Python выполняет обычный код сверху вниз. Поэтому переменную нужно создать до строки, " +
            "которая её использует. Частая ошибка — перепутать направление присваивания и написать " +
            "18 = age: слева должно находиться имя, которому присваивают значение.",
        groundingMode = GroundingMode.AI_KNOWLEDGE,
        activities = listOf(
            LearningActivity(
