    ...
    private fun validLessonJson(sourceChunkId: String = "") = """
        {
          ...
          "introduction":"SELECT читает данные из таблицы и возвращает результирующий набор строк. В простейшем запросе после SELECT перечисляют нужные столбцы, а после FROM указывают таблицу. Например, SELECT name FROM users вернёт значения столбца name для всех пользователей. Условие WHERE ограничивает строки до формирования результата, поэтому SELECT name FROM users WHERE active = 1 оставит только активных пользователей. Важно не путать выборку с изменением данных: SELECT ничего не записывает и подходит для безопасного анализа. Частая ошибка — использовать имя столбца или таблицы, которых нет в схеме.",
          "activities":[
            {
              "kind":"SINGLE_CHOICE",
