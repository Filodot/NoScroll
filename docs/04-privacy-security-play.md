...
- сетевые запросы только по явному действию пользователя при AI-генерации;
...
| Учебные материалы и планы | файл/тема пользователя | локальный курс и повторение | Room, app-private | до удаления курса/приложения |
| Фрагменты материала | явная AI-генерация | построение плана/урока | передаются выбранному AI-провайдеру, NoScroll не ведёт сетевой журнал | по правилам провайдера |
| API-ключи | ввод пользователя | доступ к Gemini/Groq/OpenRouter | AES-GCM, ключ шифрования Android Keystore | до очистки ключа/приложения |
...
Нейросеть генерирует только планы и содержимое уроков после явного действия пользователя. AI не
может решать, какие accessibility actions выполнять, какие приложения контролировать, когда
включать gate или обходить ограничения. В момент блокировки сеть не вызывается: используется
только заранее проверенное локальное задание.

## 8. Data Safety и privacy policy

Accessibility-данные остаются локальными. При AI-генерации выбранные фрагменты учебного материала
передаются внешнему провайдеру, поэтому Data Safety и privacy policy должны отражать эту передачу
по фактическим условиям каждого подключённого сервиса. Форма сверяется по
...
- crash reporting;
- remote analytics;
- новый backend, AI-провайдер или изменение передаваемого AI payload;
- cloud backup;
- support upload;
...
- [ ] Production manifest не содержит broad package visibility и лишних permissions; `INTERNET` обоснован AI-функцией.
- [ ] Accessibility package filter ограничен YouTube и Instagram.
...
- [ ] Пользователь до AI-запроса видит предупреждение о передаче выбранных фрагментов внешнему провайдеру.
- [ ] Удаление курса очищает исходные фрагменты, план, уроки, попытки и mastery.
